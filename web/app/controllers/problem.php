		...
		if (!hasContestPermission($myUser, $contest)) {
			if ($contest['cur_progress'] == CONTEST_NOT_STARTED) {
				...
				if ($myUser == null || !hasRegistered($myUser, $contest)) {
					becomeMsgPage("<h1>比赛正在进行中</h1><p>很遗憾，您尚未报名。报名后再来看吧～</p>");
				} else {
					$is_in_contest = true;
...
<div class="row d-flex justify-content-center">
	<span class="badge badge-<?=$time_limit!=1000?"danger":"secondary"?> mr-1">时间限制:<?=$time_limit!=null?"$time_limit ms":"N/A"?></span>
	<span class="badge badge-<?=$memory_limit<256||$memory_limit>512?"danger":"secondary"?> mr-1">空间限制:<?=$memory_limit!=null?"$memory_limit MB":"N/A"?></span>
</div>
...
<div class="btn-group float-right" role="group" aria-label="...">
	<a role="button" class="btn btn-info" href=""><span class="glyphicon glyphicon-download"></span>什么也不下载</a>
	<a role="button" class="btn btn-info" href="/download.php?type=problem&id=<?= $problem['id'] ?>"><span class="glyphicon glyphicon-download"></span>下载下发文件</a>
...
<div class="card my-1">
	<div class="card-header collapsed" id="headerOne" data-toggle="collapse" data-target="#collapseOne" style="cursor:pointer;">
		<h5 class="mb-0">Contests using this problem</h5>
	</div>
	<div id="collapseOne" class="collapse">
		<div class="card-body">
			<?php
				$a=DB::selectAll("select * from contests_problems where problem_id='{$problem['id']}'");
				if(!count($a)) echo("No contests.");
				for ($i = 0; $i < count($a); $i++)
					echo("<a href=\"/contest/{$a[$i]["contest_id"]}\">".DB::selectFirst("select * from contests where id='{$a[$i]["contest_id"]}'")["name"]."</a><br>");
			?>
		</div>
	</div>
</div>
