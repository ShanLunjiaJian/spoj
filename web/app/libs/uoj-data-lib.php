<?php
	...
	class SyncProblemDataHandler {
		...
		private function copy_to_prepare_with_formatter($file_name) {
		...
		private function copy_file_to_prepare_with_formatter($file_name) {
			global $uojMainJudgerWorkPath;
			if (!isset($this->allow_files[$file_name]) || !is_file("{$this->upload_dir}/$file_name")) {
				throw new UOJFileNotFoundException($file_name);
			}
			$this->copy_to_prepare_with_formatter($file_name);
		}
		...
		public function handle() {
			...
			try {
				...
				if (isset($this->allow_files['require']) && is_dir("{$this->upload_dir}/require")) {
					$this->copy_to_prepare_with_formatter('require');
				}

				if(is_file("{$this->upload_dir}/statement.pdf")) $this->copy_to_prepare('statement.pdf');
				...
				if ($this->check_conf_on('use_builtin_judger')) {
					...
					for ($num = 1; $num <= $n_tests; $num++) {
						...
						$this->copy_file_to_prepare_with_formatter($input_file_name);
						$this->copy_file_to_prepare_with_formatter($output_file_name);
					}
					...
					if (!$this->check_conf_on('interaction_mode')) {
						if (isset($this->problem_conf['use_builtin_checker'])) {
							...
							$this->copy_file_to_prepare_with_formatter('chk.cpp');
					...
					if ($this->check_conf_on('submit_answer')) {
						...
						if ($this->problem['hackable']) {
							$this->copy_file_to_prepare_with_formatter('std.cpp');
							...
							$this->copy_file_to_prepare_with_formatter('val.cpp');
							$this->compile_at_prepare('val', array('need_include_header' => true));
						}
						
						if ($this->check_conf_on('interaction_mode')) {
							$this->copy_file_to_prepare_with_formatter('interactor.cpp');
