...
void set_limit(int r, double rcur, double rmax = -1)  {
	if (rmax == -1)
		rmax = rcur;
	struct rlimit l;
	if (getrlimit(r, &l) == -1) {
...
void run_child() {
	set_limit(RLIMIT_CPU, run_program_config.time_limit/1000.0, run_program_config.real_time_limit/1000.0);
...
int main(int argc, char **argv) {
	...
	{
		ofstream debug("debug.log",ios::app);
		char str[50];
		time_t now = time(NULL);
		strftime(str, 50, "%x %X", localtime(&now));
		debug<<"\n\nrun_program: recieved a new command! time: "<<str<<"\n";
