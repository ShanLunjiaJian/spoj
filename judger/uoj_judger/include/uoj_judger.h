...
const RunLimit RL_JUDGER_DEFAULT = RunLimit(60000, 1024, 128);
...
RunCompilerResult compile_c(const string &name, const string &path = work_path) {
	return run_compiler(path.c_str(), 
			"/usr/bin/gcc-12.2.0/bin/gcc", "-o", name.c_str(), "-x", "c", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", NULL);
}
...
RunCompilerResult compile_cpp14(const string &name, const string &path = work_path) {
        return run_compiler(path.c_str(),
            "/usr/bin/gcc-12.2.0/bin/g++", "-o", name.c_str(), "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", "-std=c++14", NULL);
}
RunCompilerResult compile_cpp17(const string &name, const string &path = work_path) {
        return run_compiler(path.c_str(),
            "/usr/bin/gcc-12.2.0/bin/g++", "-o", name.c_str(), "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", "-std=c++17", NULL);
}
RunCompilerResult compile_cpp20(const string &name, const string &path = work_path) {
        return run_compiler(path.c_str(),
            "/usr/bin/gcc-12.2.0/bin/g++", "-o", name.c_str(), "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", "-std=c++2a", NULL);
}
...
RunCompilerResult compile(const char *name)  {
	...
	if ((lang == "C++" || lang == "C++11" || lang == "C++14" || lang == "C++17" || lang=="C++20" || lang == "C") && has_illegal_keywords_in_file(work_path + "/" + name + ".code"))
	{
		RunCompilerResult res;
		res.type = RS_DGS;
		res.ust = -1;
...
RunCompilerResult compile_c_with_implementer(const string &name, const string &path = work_path) {
	return run_compiler(path.c_str(), 
			"/usr/bin/gcc-12.2.0/bin/gcc", "-o", name.c_str(), "implementer.c", "-x", "c", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", NULL);
}
...
RunCompilerResult compile_cpp14_with_implementer(const string &name, const string &path = work_path) {
        return run_compiler(path.c_str(),
                        "/usr/bin/gcc-12.2.0/bin/g++", "-o", name.c_str(), "implementer.cpp", "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", "-std=c++14", NULL);
}
RunCompilerResult compile_cpp17_with_implementer(const string &name, const string &path = work_path) {
        return run_compiler(path.c_str(),
                        "/usr/bin/gcc-12.2.0/bin/g++", "-o", name.c_str(), "implementer.cpp", "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", "-std=c++17", NULL);
}
RunCompilerResult compile_cpp20_with_implementer(const string &name, const string &path = work_path) {
        return run_compiler(path.c_str(),
                        "/usr/bin/gcc-12.2.0/bin/g++", "-o", name.c_str(), "implementer.cpp", "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", "-std=c++2a", NULL);
}
...
RunCompilerResult compile_with_implementer(const char *name)  {
	...
	if (lang == "C++") return compile_cpp_with_implementer(name);
	if (lang == "C++11") return compile_cpp11_with_implementer(name);
	if (lang == "C++14") return compile_cpp14_with_implementer(name);
	if (lang == "C++17") return compile_cpp17_with_implementer(name);
	if (lang == "C++20") return compile_cpp20_with_implementer(name);

	if (lang == "C") return compile_c_with_implementer(name);
	if (lang == "Pascal") return compile_pas_with_implementer(name);
